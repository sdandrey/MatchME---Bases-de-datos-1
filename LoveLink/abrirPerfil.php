<?php
	...
	$pagina = file_get_contents("Perfil2.html");
	echo $pagina;

	$personID = (int)$_GET["id"];
	$myID = $_SESSION['person_id'];

	$stid = oci_parse($conn, "begin :ret :=le.hayVisit('$personID','$myID'); end;");
	oci_bind_by_name($stid, ':ret', $r, 200);
	oci_execute($stid);
	$hayVisit = $r;
	if($hayVisit == 0){
		$stid = oci_parse($conn, "begin le.insert_visit('$personID','$myID'); end;");
    	oci_execute($stid);
    }else{
    	$stid = oci_parse($conn, "begin le.anadirVisit('$personID','$myID'); end;");
    	oci_execute($stid);
    }
	...
	echo "<script type='text/javascript'>cambiarBoton('$texto','$personID');</script>";
